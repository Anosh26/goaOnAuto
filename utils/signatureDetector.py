...
def detect_signature(image_path: str) -> dict:
    ...
    try:
        ...
        # 1. Aspect Ratio: Signatures are typically wide rectangles (w/h >= 1.25, up to 5.0)
        ...
        # Blue hue range in OpenCV: 85 to 145 (covers royal blue, ballpoint blue, dark cyan, navy)
        ...
        # Signatures have sparse stroke coverage (between 0.5% and 25% of image area)
        has_blue_strokes = (0.005 <= blue_ink_ratio <= 0.30)
        has_dark_strokes = (0.005 <= dark_ink_ratio <= 0.30)

        # 4. Check that image does NOT have large dense text paragraphs or photos
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python signatureDetector.py <image_path>")
        sys.exit(1)
